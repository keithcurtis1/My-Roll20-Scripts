...
on("change:graphic:bar1_value", function(obj, prev) {
    if (obj.get(TOKEN_CONCENTRATING_STATUS_MARKER)) {
        ...
        if (prev["bar1_value"] > obj.get("bar1_value")) {
            ...
            sendChat("TokenDamage", "/w gm &{template:npcaction} {{rname=Concentration Check}} {{description=[DC " +final_conc_DC + " Constitution](~Macros|Concentration-Check)"+ "&#10;" +"*[Toggle Concentration Marker](~Macros|CMReport)*}}");        }
    }
});
